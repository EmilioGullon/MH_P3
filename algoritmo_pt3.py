...
from funciones_apoyo import local_search_P1,calculate_fitness
import random
import numpy as np

# Trayectorias Multiples
# BMB - Busqueda Multiarranque Basica
def BMB(datos_numpy_list, etiquetas_char_lists, num_iteraciones=20):
    mejor_solucion = None
    mejor_fitness = 0
    ...
    for _ in range(num_iteraciones):
        ...
        solucion_optima = local_search_P1(datos_numpy_list, etiquetas_char_lists, solucion_inicial)
        # Calcular fitness de la solución óptima
...
def ILS(datos_numpy_list, etiquetas_char_lists, num_iteraciones=20, intensidad_mutacion=0.25, funcion = local_search_P1):
    ...
    solucion = funcion( datos_numpy_list, etiquetas_char_lists, solucion_inicial)
    ...
    for _ in range(num_iteraciones):
        ...
        solucion_optima = funcion(datos_numpy_list, etiquetas_char_lists, solucion_mutada)
...
def ES(datos_numpy_list, etiquetas_char_lists, solucion_actual = None,T0=None, Tf=1e-3, max_evaluaciones=15000,max_vecinos=None,max_exitos=None):
    # Generar solución inicial aleatoria
    if(max_vecinos is None):
        max_vecinos=10*len(datos_numpy_list[0][0])
    ...
    while T > Tf and num_evaluaciones < max_evaluaciones:
        ...
        for _ in range(max_vecinos):
            ...
            # Generar vecino
            vecino = mutacion(solucion_actual, T)
        ...
        esquema_Cauchy(T,T0,Tf, num_enfriamientos)
...
# Ejemplo de uso
delta = 0.5  # Diferencia en fitness (negativo si la nueva solución es peor)
T = 1.0  # Temperatura actual
